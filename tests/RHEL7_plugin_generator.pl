...
# ------------------------------------------------------------------------------
# NAME
#   RHEL7_plugin_generator.pl
#
# SYNOPSIS
#   RHEL7_plugin_generator.pl [options] args
#
# DESCRIPTION
#
#
...
my $GROUPTITLE   = qr/"(SRG-OS-\d+-GPOS-\d+)",/msx;
my $RULEID       = qr/"(SV-\w+)",/msx;
my $STIGID       = qr/"(RHEL-07-\d+)",/msx;
my $RULETITLE    = qr/"(.*?)",/msx;
...
my $CSVFILE = File::Spec::Functions::catdir( $FindBin::Bin,
    qw(.. stigs U_Red_Hat_Enterprise_Linux_7_V1R1.csv) );
...
TEMPLATE
    ...
    open my $OFH, '>:encoding(utf8)',
        File::Spec::Functions::catdir( $FindBin::Bin, qw(.. plugins RHEL7),
        $NEWFILE );
    print {$OFH} $TEMPLATE;
    close $OFH;
}
